...
def get_proxy_vote_keyboard(proxy_id: int, url: str, likes: int, dislikes: int,
                            bot_username: str, show_replace: bool = True, lang: str = 'ru'):
    builder = InlineKeyboardBuilder()

    builder.row(types.InlineKeyboardButton(text=t('btn_connect', lang), url=url))
    ...
    action_row = [types.InlineKeyboardButton(text=t('btn_share', lang), url=tg_share_link)]
    if show_replace:
        action_row.append(
            types.InlineKeyboardButton(text=t('btn_other_proxy', lang), callback_data=f"replace_proxy_{proxy_id}")
        )
...
def get_cabinet_main_keyboard(lang: str = 'ru'):
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text=t('btn_favorites', lang), callback_data="liked_proxies_list"))
...
def get_proxy_manage_keyboard(proxy_id: int, has_sponsor: bool = False, is_public: bool = True,
    ...
    if is_boosted:
        builder.button(text=t('btn_boost_extend', lang), callback_data=f"buy_boost_{proxy_id}")
    else:
        builder.button(text=t('btn_boost', lang), callback_data=f"buy_boost_{proxy_id}")

    if has_sponsor:
        builder.button(text=t('btn_manage_op', lang), callback_data=f"manage_sponsor_{proxy_id}")
    else:
        builder.button(text=t('btn_buy_op', lang), callback_data=f"sponsor_menu_{proxy_id}")

    builder.button(
        text=t('visibility_yes' if is_public else 'visibility_no', lang),
        callback_data=f"toggle_public_{proxy_id}"
    )
    builder.button(text=t('btn_delete', lang), callback_data=f"user_delete_prx_{proxy_id}")
